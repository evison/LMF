            ...
            real_t *tpwgts, real_t *ubvec)
{
  ...
  switch (optype) {
    ...
    /* evison */
    case METIS_OP_BMETIS:
   	  /*User specific parameters*/
      ctrl->ctype    = GETOPTION(options, METIS_OPTION_CTYPE,    METIS_CTYPE_SHEM);
      ctrl->iptype   = GETOPTION(options, METIS_OPTION_IPTYPE,   /*METIS_IPTYPE_EDGE*/ METIS_IPTYPE_NODE); /*TODO*/
      ctrl->rtype    = GETOPTION(options, METIS_OPTION_RTYPE,    METIS_RTYPE_SEP1SIDED);
      ctrl->compress = GETOPTION(options, METIS_OPTION_COMPRESS, 1);
      ctrl->ccorder  = GETOPTION(options, METIS_OPTION_CCORDER,  0);
      ctrl->seed     = GETOPTION(options, METIS_OPTION_SEED,     -1);
      ctrl->dbglvl   = GETOPTION(options, METIS_OPTION_DBGLVL,   0);
	  ctrl->density  = GETOPTION(options, METIS_OPTION_DENSITY,  -1);
	  ctrl->density /= DIVIDER;
	  ctrl->nrows    = GETOPTION(options, METIS_OPTION_NROWS, -1);
	  ctrl->ncols    = GETOPTION(options, METIS_OPTION_NCOLS, -1);
	  ctrl->kappa    = GETOPTION(options, METIS_OPTION_KAPPA, 1);
	  ctrl->ndiags   = GETOPTION(options, METIS_OPTION_NDIAGS, -1);

	  /*Inner parameters*/
      ctrl->ufactor  = GETOPTION(options, METIS_OPTION_UFACTOR,  OMETIS_DEFAULT_UFACTOR);	/*TODO*/
	  ctrl->pfactor  = 0.1*GETOPTION(options, METIS_OPTION_PFACTOR,  0);
	  /*ncuts not used*/
	  ctrl->nseps    = GETOPTION(options, METIS_OPTION_NSEPS,    1);
      ctrl->niter    = GETOPTION(options, METIS_OPTION_NITER,    10	/*1*/);	/*TODO*/
      ctrl->objtype  = GETOPTION(options, METIS_OPTION_OBJTYPE,  METIS_OBJTYPE_NODE);
  ...
  if (ctrl->optype != METIS_OP_OMETIS && ctrl->optype != METIS_OP_BMETIS) {
  ...
  else if (ctrl->optype == METIS_OP_OMETIS){  /* METIS_OP_OMETIS */
...
int CheckParams(ctrl_t *ctrl)
{
  ...
  switch (ctrl->optype) {
    ...
    case METIS_OP_BMETIS:
      ...
      /* evison */
      if (ctrl->density <= 0) {
    	IFSET(dbglvl, METIS_DBG_INFO, printf("Input Error: Incorrect density.\n"));
        return 0;
      }
	  if (ctrl->nrows < 0){
		IFSET(dbglvl, METIS_DBG_INFO, printf("Input Error: Incorrect nrows.\n"));
        return 0;
	  }
	  if(ctrl->ncols < 0){
		IFSET(dbglvl, METIS_DBG_INFO, printf("Input Error: Incorrect ncols.\n"));
        return 0;
	  }
	  if(ctrl->kappa <= 0){
		IFSET(dbglvl, METIS_DBG_INFO, printf("Input Error: Incorrect kappa.\n"));
        return 0;
	  }
	  if(ctrl->ndiags == 0){
		  IFSET(dbglvl, METIS_DBG_INFO, printf("Input Error: Incorrect ndiags.\n"));
		  return 0;
	  }
